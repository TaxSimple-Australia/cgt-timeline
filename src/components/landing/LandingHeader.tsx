...
export default function LandingHeader() {
  ...
  return (
    <>
      {/* Main Header */}
      <header className="fixed top-0 left-0 right-0 z-40 bg-slate-300/80 backdrop-blur-sm dark:bg-slate-900">
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <Link href="/" className="flex-shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h1 className="font-bold text-xl md:text-2xl text-slate-900 dark:text-white">
                  CGT Brain AI
                </h1>
              </div>
            </Link>
              ...
              {navLinks.map((link) => {
                ...
                return link.type === 'link' ? (
                  ...
                  <button
                    ...
                    className="text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors text-base font-medium uppercase"
                  >
                    {link.label}
                  </button>
                );
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden bg-slate-300/80 backdrop-blur-sm dark:bg-slate-900 border-t border-slate-400 dark:border-slate-800">
              ...
              {navLinks.map((link) => {
                ...
                return link.type === 'link' ? (
                  ...
                  <button
                    key={link.id}
                    onClick={() => scrollToSection(link.id)}
                    className="block w-full text-left px-4 py-3 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors text-base font-medium uppercase"
                  >
                    {link.label}
                  </button>
                );

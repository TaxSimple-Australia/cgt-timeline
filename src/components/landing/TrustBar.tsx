...
export default function TrustBar() {
  ...
  return (
    <section className="relative py-16 px-4 bg-gray-50 dark:bg-slate-900 border-y border-gray-200 dark:border-slate-800">
      {/* Background gradient */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {stats.map((stat, index) => (
            ...
            >
              <div className="bg-gray-100/50 dark:bg-slate-800/50 backdrop-blur-sm border border-gray-300/50 dark:border-slate-700/50 rounded-2xl p-6 hover:border-cyan-500/30 transition-all duration-300">
        ...
        >
          {trustBadges.map((badge, index) => (
            <div
              key={index}
              className="flex items-center gap-2 px-4 py-2 bg-gray-100/30 dark:bg-slate-800/30 border border-gray-300/50 dark:border-slate-700/50 rounded-lg"
            >
              <span className="text-green-400">{badge.icon}</span>
              <span className="text-gray-600 dark:text-slate-300 text-sm font-medium">{badge.text}</span>
            </div>
          ))}
        </motion.div>

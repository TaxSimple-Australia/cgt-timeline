  ...
  return (
    <div className={`markdown-content ${className}`}>
      <ReactMarkdown
        ...
        components={{
          // Headings - pure white in dark mode
          h1: ({ node, ...props }) => (
            <h1 className="text-2xl font-bold mb-3 mt-4 pb-2 border-b border-gray-300 dark:border-gray-700 text-gray-900 dark:!text-white" {...props} />
          ),
          h2: ({ node, ...props }) => (
            <h2 className="text-xl font-bold mb-2 mt-4 pb-1 border-b border-gray-200 dark:border-gray-700 text-gray-900 dark:!text-white" {...props} />
          ),
          h3: ({ node, ...props }) => (
          ...
          // Code blocks - pure white in dark mode
          code: ({ node, inline, className, children, ...props }: any) => {
            return inline ? (
              ...
              <code className="block bg-black/5 dark:bg-white/5 p-3 rounded-lg overflow-x-auto text-sm font-mono mb-3 border border-gray-300 dark:border-gray-700 text-gray-800 dark:!text-white" {...props}>
                {children}
              </code>
            );
          },
          ...
          // Blockquotes - pure white in dark mode
          blockquote: ({ node, ...props }) => (
            <blockquote className="border-l-4 border-gray-400 dark:border-gray-600 pl-4 py-2 mb-3 italic bg-black/5 dark:bg-white/5 rounded-r text-gray-800 dark:!text-white" {...props} />
          ),

          // Tables - pure white in dark mode
          table: ({ node, ...props }) => (
            <div className="overflow-x-auto mb-4 rounded-lg border border-gray-300 dark:border-gray-700">
          ...
          // Strong/Bold - pure white in dark mode
          strong: ({ node, ...props }) => (
            <strong className="font-bold text-gray-900 dark:!text-white" {...props} />
          ),

          // Emphasis/Italic - pure white in dark mode
          em: ({ node, ...props }) => (

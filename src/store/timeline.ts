...
export interface TimelineEvent {
  ...
  // NEW: Division 40 & 43 Tax Deductions (for sale events)
  division40Assets?: number;         // Division 40 depreciating assets value claimed
  division43Deductions?: number;     // Division 43 capital works deductions claimed
  totalIncomeFromOtherSources?: number;  // Total income from other sources (salary, rental, business, etc.) for the financial year
...
export const useTimelineStore = create<TimelineState>((set, get) => {
  ...
  updateEvent: (id, updates) => {
    ...
    // Store original date for linked event detection
    ...
    // Handle all linked events - Move secondary events when primary event is dragged
    if (updates.date && event.checkboxState) {
      const currentState = get();
      const newDate = updates.date;
      const checkboxState = event.checkboxState;

      // Helper function to find and move a linked event
      const findAndMoveLinkedEvent = (linkedEventType: EventType, checkboxName: string, additionalFilter?: (e: TimelineEvent) => boolean) => {
        const linkedEvent = currentState.events.find(
          (e) =>
            e.propertyId === event.propertyId &&
            e.type === linkedEventType &&
            e.date.getTime() === originalDate.getTime() && // Same as old primary event date
            (additionalFilter ? additionalFilter(e) : true)
        );

        if (linkedEvent) {
          console.log(`🔗 Moving linked ${linkedEventType} event:`, {
            primaryEventId: event.id,
            primaryEventType: event.type,
          ...
          // Update the linked event to the new date
          set((state) => ({
            events: state.events.map((e) => {
              if (e.id !== linkedEvent.id) return e;
      ...
      if (event.type === 'purchase' && checkboxState.moveInOnSameDay) {
        findAndMoveLinkedEvent('move_in', 'moveInOnSameDay');
      }
      ...
      if (event.type === 'purchase' && checkboxState.purchaseAsVacant) {
        findAndMoveLinkedEvent('status_change', 'purchaseAsVacant', (e) => e.newStatus === 'vacant');
      }
      if (event.type === 'move_out' && checkboxState.moveOutAsVacant) {
        findAndMoveLinkedEvent('status_change', 'moveOutAsVacant', (e) => e.newStatus === 'vacant');
      }
      if (event.type === 'rent_end' && checkboxState.rentEndAsVacant) {
        findAndMoveLinkedEvent('status_change', 'rentEndAsVacant', (e) => e.newStatus === 'vacant');
      }
      ...
      if (event.type === 'purchase' && checkboxState.purchaseAsMixedUse) {
        ...
        if (hasLivingPercentage && !checkboxState.moveInOnSameDay) {
          ...
          findAndMoveLinkedEvent('move_in', 'purchaseAsMixedUse (living)');
        } else if (hasOnlyRentalPercentage) {
          // Mixed-use with only rental creates rent_start
          findAndMoveLinkedEvent('rent_start', 'purchaseAsMixedUse (rental only)');
        }
      }
    }
  },
    ...
    if (eventToDelete?.type === 'sale') {
      ...
      if (eventToDelete) {
        ...
        if (eventToDelete.type === 'status_change' && eventToDelete.newStatus === 'vacant') {
          // Could be companion of: purchase.purchaseAsVacant, move_out.moveOutAsVacant, rent_end.rentEndAsVacant, mixed_use_end.mixedUseEndAsVacant
          ...
          const p4 = findParent('mixed_use_end', 'mixedUseEndAsVacant');
          if (p4) parentUpdates.push({ parentId: p4.id, checkboxKey: 'mixedUseEndAsVacant' });
        }
  ...
  moveEvent: (id, newPosition) => {
    ...
    // Store the original date for linked event detection
    const originalDate = event.date;
    ...
    if (event.checkboxState) {
      ...
      if (event.type === 'mixed_use_end' && checkboxState.mixedUseEndAsVacant) {
